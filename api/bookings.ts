...
<tr><td style="padding:24px;text-align:center;font-size:13px;color:#8a7a5c;">Rue Liotard 4, 1202 Genève · <a href="tel:+41225034186" style="color:#d4af37;">+1 234 567 890</a> · <a href="mailto:info@testrestaurant.com" style="color:#d4af37;">info@testrestaurant.com</a></td></tr>
      ...
      if (status === "cancelled" && (previousStatus === "request" || previousStatus === "pending") && rowAfter.email) {
        ...
        if (resendKey) {
          ...
          const { data: sendData, error: sendErr } = await resend.emails.send({
            from: FROM,
            to: [rowAfter.email],
            subject: `TESTRESTAURANT – Demande de réservation`,
