...
const BCC = "info@testrestaurant.com";
...
<tr><td style="padding:24px;text-align:center;font-size:13px;color:#8a7a5c;">123 Main Street, Cityville · <a href="tel:+41225034186" style="color:#d4af37;">+1 234 567 890</a> · <a href="mailto:info@testrestaurant.com" style="color:#d4af37;">info@testrestaurant.com</a></td></tr>
  ...
  try {
    ...
    if (requestOnly) {
      const { data: sendData, error: err1 } = await resend.emails.send({
        ...
        to: [email],
        bcc: [BCC],
        subject: isValentines ? `Demande en attente – TESTRESTAURANT Geneva` : `Booking Request - ${name}`,
      ...
      const { data: sendData, error: err1 } = await resend.emails.send({
        from: FROM,
        to: [email],
        subject: `Réservation confirmée – TESTRESTAURANT Genève`,
      ...
      if (restaurantEmail) {
        const { error: err2 } = await resend.emails.send({
          from: FROM,
          to: [restaurantEmail],
          subject: `[TESTRESTAURANT] Nouvelle réservation (confirmée) : ${name} – ${date} ${time}`,
    ...
    if (requestOnly && restaurantEmail) {
      const { error: err2 } = await resend.emails.send({
        from: FROM,
        to: [restaurantEmail],
        subject: `[TESTRESTAURANT] Demande de réservation : ${name} – ${date} ${time}`,
        html: restaurantEmailHtml(data),
      });
      if (err2) console.error("[booking] Restaurant email failed:", err2);
    }
    ...
    sendPushToAllSubscriptions({
      ...
      url: "/admin",
      tag: "TESTRESTAURANT-new-booking",
